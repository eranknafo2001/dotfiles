{...}: {
  flake.homeModules.ai-opencode = {
    ...
    btcaRepos =
      if lib.hasAttrByPath ["my" "btca" "repos"] options
      then config.my.btca.repos
      else [];
    btcaPrompt = lib.optionalString (btcaRepos != []) ''
      ## btca
    ...
    '';
    opencode-wrapper = lib.mkSecretWrapper pkgs.llm-agents.opencode [
      {
